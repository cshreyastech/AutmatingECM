...
float Utilities::GetAngle(Vector3f vec_a, Vector3f vec_b, Vector3f up_vector) {
  float angle = 0.0;
  vec_a.normalize();
  vec_b.normalize();
...
float Utilities::GetRandomBetweenRange(float low, float high) {
  if(high < low) {
...
Matrix3f Utilities::RotationFromEuler(Vector3f R_rpy) {
  float roll = R_rpy(0), pitch = R_rpy(1), yaw = R_rpy(2);
...
Vector3f Utilities::RPYfromRotation(Matrix3f R) {
  Vector3f rpy;
...
Matrix4f Utilities::GetFrame(Matrix3f R, Vector3f T){
  Matrix4f Trans;
  Trans.setIdentity();
